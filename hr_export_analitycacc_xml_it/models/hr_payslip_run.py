		...
		for i in range(len(datos_planilla)):
			...
			for j in range(range_row):
				...
				if j in [0,1,2,3,4,5,6,7]:
					worksheet.write(x, j, datos_planilla[i][j] if datos_planilla[i][j] else '', formatLeft)
				else:
					worksheet.write(x, j, datos_planilla[i][j] if datos_planilla[i][j] else '0.00', numberdos)
			essalud = self.env['hr.payslip'].browse(datos_planilla[i][7]).essalud
			worksheet.write(x,j+1,essalud,numberdos)
			total_essalud += essalud
			x = x+1
		x = x + 1
		datos_planilla_transpuesta = zip(*datos_planilla)
		for j in range(8, len(datos_planilla_transpuesta)):
...
class PlanillaPlanillaTabularWizard(models.TransientModel):
	...
	def reconstruye_tabla(self,date_start,date_end):
		...
		fields += 'x_date_start date,'

		self.env['ir.model.fields'].create({
			'model_id': im.id,
			'name': 'x_date_start',
			'field_description': u'FECHA DE INGRESO',
			'ttype': 'date',
			'state': 'manual',
		})
		...
		fields += 'x_date_end date,'
		self.env['ir.model.fields'].create({
			'model_id': im.id,
			'name': 'x_date_end',
			'field_description': u'FECHA DE CESE',
			'ttype': 'date',
			'state': 'manual',
		})
		etree.SubElement(start_concepts, "field",
		...
		SELECT * FROM crosstab(
			$$ 
				...
				group by t.name_related,hp.employee_id,t.sequence,t.codigo,t.date_start,t.date_end
				order by 1,2
			$$,
				$$  select sr.sequence
					from hr_salary_rule sr
