...
left join(select account_id,sum(debit)-sum(credit) as saldo00 from account_move_line aml inner join account_move am on am.id = aml.move_id  where am.state != 'draft' and extract(year from am.fecha_contable)='"""+str(self.fiscalyear_id.name)+"""' and extract(month from am.fecha_contable)='01' and coalesce(am.fecha_special,false) = true group by account_id) t0 on t0.account_id=c1.id
left join(select account_id,sum(debit)-sum(credit) as saldo01 from account_move_line aml inner join account_move am on am.id = aml.move_id  where am.state != 'draft' and extract(year from am.fecha_contable)='"""+str(self.fiscalyear_id.name)+"""' and extract(month from am.fecha_contable)='01' and coalesce(am.fecha_special,false) = false group by account_id) t1 on t1.account_id=c1.id
left join(select account_id,sum(debit)-sum(credit) as saldo02 from account_move_line aml inner join account_move am on am.id = aml.move_id  where am.state != 'draft' and extract(year from am.fecha_contable)='"""+str(self.fiscalyear_id.name)+"""' and extract(month from am.fecha_contable)='02' group by account_id) t3 on t3.account_id=c1.id
left join(select account_id,sum(debit)-sum(credit) as saldo03 from account_move_line aml inner join account_move am on am.id = aml.move_id  where am.state != 'draft' and extract(year from am.fecha_contable)='"""+str(self.fiscalyear_id.name)+"""' and extract(month from am.fecha_contable)='03' group by account_id) t4 on t4.account_id=c1.id
left join(select account_id,sum(debit)-sum(credit) as saldo04 from account_move_line aml inner join account_move am on am.id = aml.move_id  where am.state != 'draft' and extract(year from am.fecha_contable)='"""+str(self.fiscalyear_id.name)+"""' and extract(month from am.fecha_contable)='04' group by account_id) t5 on t5.account_id=c1.id
left join(select account_id,sum(debit)-sum(credit) as saldo05 from account_move_line aml inner join account_move am on am.id = aml.move_id  where am.state != 'draft' and extract(year from am.fecha_contable)='"""+str(self.fiscalyear_id.name)+"""' and extract(month from am.fecha_contable)='05' group by account_id) t6 on t6.account_id=c1.id
left join(select account_id,sum(debit)-sum(credit) as saldo06 from account_move_line aml inner join account_move am on am.id = aml.move_id  where am.state != 'draft' and extract(year from am.fecha_contable)='"""+str(self.fiscalyear_id.name)+"""' and extract(month from am.fecha_contable)='06' group by account_id) t7 on t7.account_id=c1.id
left join(select account_id,sum(debit)-sum(credit) as saldo07 from account_move_line aml inner join account_move am on am.id = aml.move_id  where am.state != 'draft' and extract(year from am.fecha_contable)='"""+str(self.fiscalyear_id.name)+"""' and extract(month from am.fecha_contable)='07' group by account_id) t8 on t8.account_id=c1.id
left join(select account_id,sum(debit)-sum(credit) as saldo08 from account_move_line aml inner join account_move am on am.id = aml.move_id  where am.state != 'draft' and extract(year from am.fecha_contable)='"""+str(self.fiscalyear_id.name)+"""' and extract(month from am.fecha_contable)='08' group by account_id) t9 on t9.account_id=c1.id
left join(select account_id,sum(debit)-sum(credit) as saldo09 from account_move_line aml inner join account_move am on am.id = aml.move_id  where am.state != 'draft' and extract(year from am.fecha_contable)='"""+str(self.fiscalyear_id.name)+"""' and extract(month from am.fecha_contable)='09' group by account_id) t10 on t10.account_id=c1.id
left join(select account_id,sum(debit)-sum(credit) as saldo10 from account_move_line aml inner join account_move am on am.id = aml.move_id  where am.state != 'draft' and extract(year from am.fecha_contable)='"""+str(self.fiscalyear_id.name)+"""' and extract(month from am.fecha_contable)='10' group by account_id) t11 on t11.account_id=c1.id
left join(select account_id,sum(debit)-sum(credit) as saldo11 from account_move_line aml inner join account_move am on am.id = aml.move_id  where am.state != 'draft' and extract(year from am.fecha_contable)='"""+str(self.fiscalyear_id.name)+"""' and extract(month from am.fecha_contable)='11' group by account_id) t12 on t12.account_id=c1.id
left join(select account_id,sum(debit)-sum(credit) as saldo12 from account_move_line aml inner join account_move am on am.id = aml.move_id  where am.state != 'draft' and extract(year from am.fecha_contable)='"""+str(self.fiscalyear_id.name)+"""' and extract(month from am.fecha_contable)='12' group by account_id) t13 on t13.account_id=c1.id
