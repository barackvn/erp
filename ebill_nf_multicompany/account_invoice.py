...
class empresa_empresa(models.Model):
	...
	db = fields.Char(string='DB')
	url = fields.Char(string='URL NUBEFACT')
	token = fields.Char(string='TOKEN NUBEFACT')


class account_invoice(models.Model):
	_inherit='account.invoice'

	empresa = fields.Many2one('empresa.empresa',string="Compania",required=True)
	...
	def make_einvoice(self):
		...
		if self[0].journal_id.type == 'sale' or self[0].journal_id.type == 'sale_refund':
			for self_act in self:
				...
				_logger.info(head_json)
				jsonarray = json.dumps(head_json,indent=4)
				req = urllib2.Request(self_act.empresa.url)
				req.add_header('Content-Type', 'application/json')
